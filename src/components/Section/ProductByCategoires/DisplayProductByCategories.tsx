...
export default function DisplayProductByCategories() {
  ...
  const fetchProductsGroupedByCategories = async (): Promise<
    ProductsByCategory[]
  > => {
    try {
      // Fetch all categories from the categories collection
      const categories: Array<CategoryType & { id: string }> =
        await getAllDocsFromCollection<CategoryType>("categories");
      ...
      const products: Array<ProductType & { id: string }> =
        await getAllDocsFromCollection<ProductType>("products");

      // Group products by category name
      const productsByCategories: ProductsByCategory[] = visibleCategories
        .map((category) => {
          const productsForCategory = products.filter(
            (product) => product.categories.includes(category.cid) // Check if the category id exists in the product's categories array
          );
        ...
        .filter(
          (category): category is ProductsByCategory => category !== null
        ); // Filter out null and assert the correct type
  ...
  const memoizedProductsByCategories = useMemo(
